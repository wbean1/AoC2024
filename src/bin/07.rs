...
pub fn part_one(input: &str) -> Option<u64> {
    ...
    for equation in equations {
        let mut possible_answers: HashSet<u64> = HashSet::new();
        ...
        while has_unsolved_wip(&wips) {
            ...
            if unsolved_wip.len() == 2 {
                possible_answers.insert(possible_answer_1);
                possible_answers.insert(possible_answer_2);
            } else {
        ...
        if possible_answers.contains(&equation.0) {
            sum += equation.0;
        }
    }
